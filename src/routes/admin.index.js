...
import { fetchHorarios, fetchConfigEntrega, fetchAvisoLoja, DEFAULT_AVISO, DAY_LABELS, SERVICE_LABELS, } from "@/lib/store-hours";
import { PainelError } from "@/components/PainelError";
import { useServerFn } from "@tanstack/react-start";
import { listKitchenUsers, createKitchenUser, updateKitchenPermissions, deleteKitchenUser, updateKitchenPassword, } from "@/lib/kitchen-users.functions";
...
function KitchenUsersPanel() {
    ...
    const createMut = useMutation({
        mutationFn: () => create({ data: { email: newEmail, password: newPassword, permissions: newPerms } }),
        onSuccess: () => {
    ...
    const updateMut = useMutation({
        mutationFn: (v) => updatePerms({ data: v }),
        onSuccess: () => {
    ...
    const updatePassMut = useMutation({
        mutationFn: (v) => updatePass({ data: v }),
        onSuccess: () => {
    ...
    const delMut = useMutation({
        mutationFn: (user_id) => remove({ data: { user_id } }),
        onSuccess: () => {
    ...
    return (<div className="space-y-6">
      <Card className="p-4">
        ...
        <div className="mt-4 space-y-2">
          ...
          {Object.keys(PERM_LABELS).map((k) => (<label key={k} className="flex items-center gap-2 text-sm">
              <Switch checked={newPerms[k]} onCheckedChange={(v) => setNewPerms((p) => ({ ...p, [k]: v }))}/>
              {PERM_LABELS[k]}
            </label>))}
        </div>
        <div className="mt-4 flex justify-end">
          <Button onClick={() => createMut.mutate()} disabled={createMut.isPending || !newEmail || newPassword.length < 6}>
      ...
      <div>
        ...
        {isLoading ? <Loader2 className="h-6 w-6 animate-spin"/> : (<div className="grid gap-3">
            {(users ?? []).map((u) => (<Card key={u.id} className="p-4">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {Object.keys(PERM_LABELS).map((k) => (<label key={k} className="flex items-center gap-2 text-sm">
                      <Switch checked={!!u.permissions?.[k]} onCheckedChange={(v) => updateMut.mutate({
                        user_id: u.id,
            ...
            {(users ?? []).length === 0 && (<p className="text-sm text-muted-foreground">Nenhum usuário da cozinha cadastrado.</p>)}
          </div>)}
      </div>
    </div>);
}
function PasswordDialog({ user, onUpdate, isPending }) {
    ...
    return (<Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button disabled={isPending || password.length < 6} onClick={() => {
            onUpdate(password);
            setPassword("");
            setOpen(false);
        }}>
...
function CategoriesPanel() {
    ...
    return (<div>
      ...
      {isLoading ? <Loader2 className="h-6 w-6 animate-spin"/> : (<div className="grid gap-2">
          {data?.map((c) => (<Card key={c.id} className="p-3 flex items-center justify-between">
              <div>
                <div className="font-medium">{c.name}</div>
                <div className="text-xs text-muted-foreground">
                  Ordem: {c.sort_order} · {c.available_lunch ? "Almoço ✓" : "Almoço ✗"} · {c.available_dinner ? "Churrasco ✓" : "Churrasco ✗"}
                </div>
              </div>
              <div className="flex gap-2">
...
function CategoryDialog({ category, trigger }) {
    const qc = useQueryClient();
    const [open, setOpen] = useState(false);
    const [name, setName] = useState(category?.name ?? "");
    const [sortOrder, setSortOrder] = useState(category?.sort_order ?? 0);
    const [availLunch, setAvailLunch] = useState(category?.available_lunch ?? true);
    const [availDinner, setAvailDinner] = useState(category?.available_dinner ?? true);
    useEffect(() => {
        if (open) {
            setName(category?.name ?? "");
            setSortOrder(category?.sort_order ?? 0);
            setAvailLunch(category?.available_lunch ?? true);
            setAvailDinner(category?.available_dinner ?? true);
        }
    }, [open, category]);
    const save = useMutation({
        mutationFn: async () => {
            const payload = { name, sort_order: sortOrder, available_lunch: availLunch, available_dinner: availDinner };
            if (category) {
...
function ItemsPanel() {
    ...
    return (<div>
      ...
      {isLoading ? <Loader2 className="h-6 w-6 animate-spin"/> : (<div className="space-y-6">
          {cats?.map((c) => (<div key={c.id}>
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-2">{c.name}</h3>
              <div className="grid gap-2">
                {(byCat[c.id] ?? []).map((it) => (<Card key={it.id} className="p-3 flex items-center justify-between gap-3">
...
function ItemDialog({ item, categories, trigger }) {
    const qc = useQueryClient();
    const [open, setOpen] = useState(false);
          ...
          <div className="rounded-lg border border-border p-3 space-y-3">
            ...
            <div className="flex items-start gap-2 pt-2 border-t border-border">
              ...
              <div className="flex-1">
                ...
                {hasExtraQuestion && (<div className="mt-3 space-y-3">
                    <div>
                      <Label className="text-xs">Texto da Pergunta</Label>
                      <Input value={extraQuestionText} onChange={(e) => setExtraQuestionText(e.target.value)} placeholder="Ex: Acompanha mel?" className="h-8 text-sm" autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
                    </div>
                    <div>
                      <Label className="text-xs">Opções (separadas por vírgula)</Label>
                      <Input value={extraQuestionOptions} onChange={(e) => setExtraQuestionOptions(e.target.value)} placeholder="Ex: Sim, Não" className="h-8 text-sm" autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
...
function NeighborhoodsPanel() {
    ...
    return (<div>
      ...
      {isLoading ? <Loader2 className="h-6 w-6 animate-spin"/> : (<div className="grid gap-2">
          {data?.map((n) => (<Card key={n.id} className="p-3 flex items-center justify-between">
              <div>
...
function AcompanhamentosPanel() {
    ...
    return (<div>
      ...
      {isLoading ? <Loader2 className="h-6 w-6 animate-spin"/> : (<div className="grid gap-2">
          {data?.map((a) => (<Card key={a.id} className="p-3 flex items-center justify-between">
              <div>
...
function AcompanhamentoDialog({ acompanhamento, trigger }) {
    const qc = useQueryClient();
    const [open, setOpen] = useState(false);
...
function NeighborhoodDialog({ neighborhood, trigger }) {
    const qc = useQueryClient();
    const [open, setOpen] = useState(false);
...
function SettingsPanel() {
    ...
    useEffect(() => { if (data)
        setForm({ ...data, report_emails: (data.report_emails ?? []).join(", ") }); }, [data]);
    const save = useMutation({
        mutationFn: async () => {
            const payload = {
...
function HorariosPanel() {
    ...
    useEffect(() => { if (horarios)
        setRows(horarios); }, [horarios]);
    useEffect(() => { if (entrega)
        setPrazo(entrega); }, [entrega]);
    ...
    if (isLoading || !rows)
        return <Loader2 className="h-5 w-5 animate-spin mt-6"/>;
    const update = (id, patch) => setRows((prev) => (prev ? prev.map((r) => (r.id === id ? { ...r, ...patch } : r)) : prev));
    ...
    return (<>
      <Card className="p-6 max-w-3xl space-y-4 mt-6">
        ...
        <div className="space-y-2">
          {ordered.map((r) => (<div key={r.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-border p-3 sm:flex sm:flex-wrap">
              ...
              <div className="flex items-center gap-2">
                <Input type="time" className="w-[110px]" value={r.hora_abertura.slice(0, 5)} onChange={(e) => update(r.id, { hora_abertura: e.target.value })}/>
                <span className="text-muted-foreground">às</span>
                <Input type="time" className="w-[110px]" value={r.hora_fechamento.slice(0, 5)} onChange={(e) => update(r.id, { hora_fechamento: e.target.value })}/>
              </div>
              <label className="col-span-2 flex items-center gap-2 text-sm sm:col-auto">
                <Switch checked={r.delivery_disponivel} onCheckedChange={(v) => update(r.id, { delivery_disponivel: v })}/>
                Delivery
              </label>
            </div>))}
        </div>
      ...
      {prazo && (<Card className="p-6 max-w-2xl space-y-4 mt-6">
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="number" value={prazo.prazo_minimo_minutos} onChange={(e) => setPrazo({ ...prazo, prazo_minimo_minutos: Number(e.target.value) })}/>
            </div>
            <div>
              <Label>Prazo máximo (min)</Label>
              <Input type="number" value={prazo.prazo_maximo_minutos} onChange={(e) => setPrazo({ ...prazo, prazo_maximo_minutos: Number(e.target.value) })}/>
            </div>
          </div>
          <div>
            <Label>Observação exibida abaixo</Label>
            <Input value={prazo.texto_observacao} onChange={(e) => setPrazo({ ...prazo, texto_observacao: e.target.value })} autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
          </div>
...
function AvisoFechadoPanel() {
    ...
    if (!aviso)
        return null;
    return (<Card className="p-6 max-w-2xl space-y-4 mt-6">
      <div>
      ...
      <div>
        ...
        <Input value={aviso.titulo_fechado} onChange={(e) => setAviso({ ...aviso, titulo_fechado: e.target.value })} placeholder={DEFAULT_AVISO.titulo_fechado} autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
      </div>
      <label className="flex items-center gap-3 text-sm">
        <Switch checked={aviso.horarios_modo === "manual"} onCheckedChange={(v) => setAviso({ ...aviso, horarios_modo: v ? "manual" : "auto" })}/>
        Escrever os horários manualmente (desligado = gerado automaticamente pelos horários acima)
      </label>
      {aviso.horarios_modo === "manual" && (<div>
          <Label>Texto dos horários no aviso</Label>
          <Textarea rows={3} value={aviso.horarios_texto} onChange={(e) => setAviso({ ...aviso, horarios_texto: e.target.value })} placeholder="Almoço: Seg–Sáb 11h às 14h30 · Churrasquinho: Seg–Sáb 18h às 00h · Dom 11h às 00h" autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
        </div>)}
      <div className="border-t border-border pt-4">
        <Label>Título do cartão de horários (página inicial)</Label>
        <Input value={aviso.home_horario_titulo} onChange={(e) => setAviso({ ...aviso, home_horario_titulo: e.target.value })} placeholder={DEFAULT_AVISO.home_horario_titulo} autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
      </div>
      <div>
        <Label>Texto do cartão de horários (página inicial)</Label>
        <Textarea rows={4} value={aviso.home_horario_texto} onChange={(e) => setAviso({ ...aviso, home_horario_texto: e.target.value })} placeholder={DEFAULT_AVISO.home_horario_texto} autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
        <p className="mt-1 text-xs text-muted-foreground">
      ...
      <div className="border-t border-border pt-4 space-y-4">
        ...
        <div>
          ...
          <Input value={aviso.order_confirmation_message} onChange={(e) => setAviso({ ...aviso, order_confirmation_message: e.target.value })} placeholder={DEFAULT_AVISO.order_confirmation_message} autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
        </div>
        <div>
          <Label>Tempo estimado</Label>
          <Input value={aviso.order_estimated_time} onChange={(e) => setAviso({ ...aviso, order_estimated_time: e.target.value })} placeholder={DEFAULT_AVISO.order_estimated_time} autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
        </div>
      </div>
...
function ReservationsPanel() {
    ...
    const filtered = (data ?? []).filter((r) => {
        if (searchName && !r.customer_name.toLowerCase().includes(searchName.toLowerCase()))
            return false;
        if (searchPhone && !r.phone.includes(searchPhone))
            return false;
        if (filterDate && r.reservation_date !== filterDate)
            return false;
        return true;
    });
    return (<div className="space-y-4">
      ...
      {isLoading ? (<Loader2 className="h-6 w-6 animate-spin"/>) : (<div className="grid gap-3">
          ...
          {filtered.length === 0 && (<p className="text-sm text-muted-foreground">Nenhuma reserva encontrada.</p>)}
        </div>)}
    </div>);
}
/* --------------------------- REVIEWS --------------------------- */
function ReviewsPanel() {
    ...
    return (<div className="space-y-3">
      {isLoading ? (<Loader2 className="h-6 w-6 animate-spin"/>) : (data ?? []).length === 0 ? (<p className="text-sm text-muted-foreground">Nenhuma avaliação recebida ainda.</p>) : ((data ?? []).map((r) => (<Card key={r.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-0.5 mb-1">
                  {[1, 2, 3, 4, 5].map((n) => (<Star key={n} className={`h-4 w-4 ${n <= r.rating ? "fill-primary text-primary" : "text-muted-foreground/30"}`}/>))}
                  <span className="ml-2 text-xs text-muted-foreground">
                    {new Date(r.created_at).toLocaleString("pt-BR")}
                  </span>
                </div>
                {r.comment ? (<p className="text-sm whitespace-pre-wrap">{r.comment}</p>) : (<p className="text-sm italic text-muted-foreground">Sem comentário</p>)}
              </div>
...
function ReceiptSettingsPanel() {
    ...
    return (<Card className="p-6 max-w-3xl space-y-6">
      <div>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch id="qty-big" checked={form.receipt_qty_double_size} onCheckedChange={(v) => setForm({ ...form, receipt_qty_double_size: v })}/>
          </div>

          <div className="flex items-center justify-between">
            <Label htmlFor="extra-spacing" className="cursor-pointer">Espaçamento extra entre itens</Label>
            <Switch id="extra-spacing" checked={form.receipt_extra_spacing} onCheckedChange={(v) => setForm({ ...form, receipt_extra_spacing: v })}/>
          </div>

          <div className="space-y-2">
            <Label>Tamanho da fonte do cabeçalho</Label>
            <Select value={String(form.receipt_font_size)} onValueChange={(v) => setForm({ ...form, receipt_font_size: Number(v) })}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch id="bold-header" checked={form.receipt_header_bold} onCheckedChange={(v) => setForm({ ...form, receipt_header_bold: v })}/>
          </div>

          <div className="flex items-center justify-between">
            <Label htmlFor="bold-items" className="cursor-pointer">Itens em Negrito</Label>
            <Switch id="bold-items" checked={form.receipt_items_bold} onCheckedChange={(v) => setForm({ ...form, receipt_items_bold: v })}/>
          </div>

          <div className="flex items-center justify-between">
            <Label htmlFor="bold-footer" className="cursor-pointer">Rodapé em Negrito</Label>
            <Switch id="bold-footer" checked={form.receipt_footer_bold} onCheckedChange={(v) => setForm({ ...form, receipt_footer_bold: v })}/>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 border-t pt-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Switch id="show-logo" checked={form.receipt_show_logo} onCheckedChange={(v) => setForm({ ...form, receipt_show_logo: v })}/>
        </div>

        <div className="space-y-2">
          <Label>URL da Logo P&B (Formato BMP/PNG monocromático recomendado)</Label>
          <Input value={form.official_logo_bw_url} onChange={(e) => setForm({ ...form, official_logo_bw_url: e.target.value })} placeholder="Link da imagem..." autoCapitalize="none" autoCorrect="off" spellCheck="false"/>
