...
import { buildDailyReportBytes, buildShiftReportBytes, sendBytesToPrinter, money, PAYMENT_LABELS as REPORT_PAYMENT_LABELS, } from "@/lib/report";
import { createDailyReport, createShiftReport, getDailyReport, getShiftReports, markPrinted, todayISO, } from "@/lib/reports-service";
...
function OperacionalError({ error, reset }) {
    if (error)
        console.error(error);
    return (<div className="min-h-screen flex items-center justify-center bg-background px-4">
      <Card className="p-8 max-w-md text-center">
        ...
        {error?.message && (<p className="mt-2 text-xs text-muted-foreground break-words">{error.message}</p>)}
        <div className="mt-4 flex justify-center gap-2">
          ...
          <Button variant="outline" onClick={() => { if (typeof window !== "undefined")
...
function KitchenDashboard() {
    ...
    const [confirmPayFor, setConfirmPayFor] = useState(null);
    const [deletingOrder, setDeletingOrder] = useState(null);
    const [deletionReason, setDeletionReason] = useState("");
    const [lastMotoboyId, setLastMotoboyId] = useState(null);
    const sendShiftEmail = useServerFn(sendShiftReportEmail);
    const deleteOrderFn = useServerFn(deleteOrder);
    useEffect(() => {
        window.extSetDeletingOrder = setDeletingOrder;
    }, []);
    ...
    const updateStatus = useMutation({
        ...
        onMutate: async ({ id, status }) => {
            ...
            qc.setQueryData(["kitchen-orders"], (old = []) => old.map((o) => (o.id === id ? { ...o, status } : o)));
            return { previous };
        },
        onError: (e, _vars, ctx) => {
            if (ctx?.previous)
                qc.setQueryData(["kitchen-orders"], ctx.previous);
            toast.error(e.message);
        },
        onSuccess: (_data, { id, status }) => {
            if (status === "delivered") {
                const list = (qc.getQueryData(["kitchen-orders"]) ?? []);
                const target = list.find((o) => o.id === id);
                if (target && !target.payment_confirmed_at && p.can_confirm_payment) {
    ...
    const confirmPayment = useMutation({
        mutationFn: async ({ id, method, motoboyId }) => {
    ...
    useEffect(() => {
        const channel = supabase
            ...
            qc.setQueryData(["kitchen-orders"], (old = []) => {
                if (old.some((o) => o.id === fullOrder.id))
                    return old;
            ...
            qc.setQueryData(["kitchen-orders"], (old = []) => old.map((o) => (o.id === updated.id ? { ...o, ...updated } : o)));
        })
            .subscribe();
        return () => {
    ...
    const shiftOrders = useMemo(() => (activeShift ? orders.filter((o) => o.shift_id === activeShift.id) : []), [orders, activeShift]);
    const shiftRevenue = useMemo(() => shiftOrders
        .filter((o) => o.payment_confirmed_at)
        .reduce((s, o) => s + Number(o.total), 0), [shiftOrders]);
    const awaitingPayment = useMemo(() => shiftOrders.filter((o) => o.status === "delivered" && !o.payment_confirmed_at), [shiftOrders]);
    const pendingActive = useMemo(() => shiftOrders.filter((o) => o.status !== "delivered"), [shiftOrders]);
    ...
    return (<div className="min-h-screen bg-background">
      <header className="border-b bg-card sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-4 py-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            {activeShift ? (<Badge className="bg-green-600 hover:bg-green-600">
                Turno {activeShift.shift_type === "almoco" ? "Almoço" : "Noite"} aberto
              </Badge>) : (<Badge variant="destructive">Nenhum turno aberto</Badge>)}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2">
              ...
              <Input id="agentUrl" value={agentUrl} onChange={(e) => setAgentUrl(e.target.value)} className="h-8 w-48 sm:w-64 text-xs" placeholder="http://localhost:8080/print"/>
            ...
            {p.can_open_close_shift && activeShift ? (<Button size="sm" variant="outline" onClick={() => setCloseShiftModal(true)} className="border-destructive text-destructive hover:bg-destructive hover:text-destructive-foreground">
                <Square className="h-4 w-4 mr-2"/> Fechar turno
              </Button>) : p.can_open_close_shift ? (<Button size="sm" onClick={() => setOpenShiftModal(true)}>
        ...
        {activeShift && (<div className="mx-auto max-w-7xl px-4 pb-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            <StatChip label="Operador" value={activeShift.operator_name ?? "—"}/>
            <StatChip label="Aberto às" value={new Date(activeShift.opened_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}/>
            <StatChip label="Caixa inicial" value={Number(activeShift.opening_cash).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}/>
            <StatChip label="Faturado (confirmado)" value={shiftRevenue.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} highlight/>
          </div>)}
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        {isLoading ? (<div className="flex justify-center py-20">
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <section>
                  ...
                  <div className="space-y-4">
                    {activeOrders.map((order) => (<OrderCard key={order.id} order={order} onStatus={(s) => updateStatus.mutate({ id: order.id, status: s })} onPrint={() => printOne(order)} onConfirmPayment={() => setConfirmPayFor(order)} isPending={updateStatus.isPending} canUpdateStatus={p.can_update_order_status} canConfirmPayment={p.can_confirm_payment}/>))}
                    {activeOrders.length === 0 && (<Card className="p-8 text-center text-muted-foreground">
                ...
                <section>
                  <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
                    ...
                    {awaitingPayment.length > 0 && (<Badge className="bg-amber-500 hover:bg-amber-500">
                        {awaitingPayment.length} aguardando pagamento
                      </Badge>)}
                  </h2>
                  <div className="space-y-4">
                    {doneOrders.slice(0, 20).map((order) => (<OrderCard key={order.id} order={order} onStatus={(s) => updateStatus.mutate({ id: order.id, status: s })} onPrint={() => printOne(order)} onConfirmPayment={() => setConfirmPayFor(order)} isPending={updateStatus.isPending} compact canUpdateStatus={p.can_update_order_status} canConfirmPayment={p.can_confirm_payment}/>))}
                    {doneOrders.length === 0 && (<Card className="p-8 text-center text-muted-foreground">
            ...
            {p.can_manage_menu && (<TabsContent value="menu" className="mt-4">
                <MenuAvailabilityPanel />
              </TabsContent>)}
          </Tabs>)}
      </main>

      <OpenShiftDialog open={openShiftModal} onClose={() => setOpenShiftModal(false)} onOpened={() => {
            setOpenShiftModal(false);
            refetchShift();
        }}/>
      <CloseShiftDialog open={closeShiftModal} shift={activeShift ?? null} canClose={canCloseShift} pendingActive={pendingActive.length} awaitingPayment={awaitingPayment.length} agentUrl={agentUrl} onClose={() => setCloseShiftModal(false)} sendShiftEmail={sendShiftEmail} onClosed={() => {
      ...
      <ConfirmPaymentDialog order={confirmPayFor} shiftId={activeShift?.id ?? null} lastMotoboyId={lastMotoboyId} onClose={() => setConfirmPayFor(null)} onConfirm={(method, motoboyId) => {
            if (!confirmPayFor)
                return;
      ...
      <Dialog open={!!deletingOrder} onOpenChange={(open) => !open && setDeletingOrder(null)}>
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <Input id="deletionReason" placeholder="Ex: Pedido duplicado, cliente cancelou, etc." value={deletionReason} onChange={(e) => setDeletionReason(e.target.value)} className="mt-2" autoFocus/>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteOrder} disabled={deletionReason.trim().length < 5}>
              Confirmar exclusão
            </Button>
          </DialogFooter>
        </DialogContent>
...
function OrderCard({ order, onStatus, onPrint, onConfirmPayment, isPending, compact, canUpdateStatus = true, canConfirmPayment = true, }) {
    const currentIndex = STATUS_FLOW.indexOf(order.status);
    const nextStatus = STATUS_FLOW[currentIndex + 1];
    const anyOrder = order;
    window.extSetDeletingOrder = window.extSetDeletingOrder || (() => { });
    const needsPayConfirm = order.status === "delivered" && !anyOrder.payment_confirmed_at;
    const payConfirmed = !!anyOrder.payment_confirmed_at;
    return (<Card className={`p-4 ${compact ? "opacity-70" : ""}`}>
      ...
      <div className="border-t pt-3 mb-3">
        ...
        <ul className="space-y-1 text-sm">
          {order.order_items.map((item) => (<li key={item.id} className="flex justify-between">
        ...
        {order.delivery_type === "delivery" && (<div className="flex justify-between text-sm">
            <span>Entrega</span>
            <span>R$ {order.delivery_fee.toFixed(2).replace(".", ",")}</span>
          </div>)}
      </div>

      {order.notes && (<div className="bg-muted rounded-md p-2 text-sm mb-3">
          <span className="font-semibold">Obs:</span> {order.notes}
        </div>)}

      {needsPayConfirm && (<div className="mb-3 rounded-md border border-amber-500/40 bg-amber-500/10 p-2 text-xs">
          Aguardando confirmação de pagamento (motoboy retornou?).
        </div>)}
      {payConfirmed && (<div className="mb-3 rounded-md border border-green-500/40 bg-green-500/10 p-2 text-xs">
          Pagamento confirmado: {PAYMENT_LABELS[anyOrder.confirmed_payment_method] ?? anyOrder.confirmed_payment_method}
        </div>)}

      {canUpdateStatus && (<div className="mb-3">
          ...
          <div className="flex items-stretch gap-1">
            {STATUS_FLOW.map((s, i) => {
                ...
                return (<button key={s} type="button" disabled={!clickable || isPending} onClick={() => clickable && onStatus(s)} className={`flex-1 rounded-md border px-1 py-1.5 text-[11px] font-semibold leading-tight transition-colors ${done
                        ? "border-primary/50 bg-primary/15 text-primary"
                        : "border-border bg-muted/30 text-muted-foreground hover:bg-primary/10 hover:text-primary"} ${clickable ? "cursor-pointer" : "cursor-default"}`}>
                  {STATUS_LABELS[s]}
                </button>);
            })}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button size="sm" variant="outline" onClick={onPrint} className="border-primary/50 hover:bg-primary/10 font-bold">
          <Printer className="h-4 w-4 mr-2"/> REIMPRIMIR
        </Button>
        {nextStatus && canUpdateStatus && (<Button size="sm" onClick={() => onStatus(nextStatus)} disabled={isPending} className={`font-black uppercase tracking-tight ${STATUS_COLORS[nextStatus] ?? ""}`}>
            {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
            {STATUS_LABELS[nextStatus]}
          </Button>)}
        <Button size="sm" variant="destructive" onClick={() => window.extSetDeletingOrder(order)} className="font-bold">
          <Trash2 className="h-4 w-4 mr-2"/> EXCLUIR
        </Button>
        {needsPayConfirm && canConfirmPayment && (<Button size="sm" onClick={onConfirmPayment} className="bg-amber-600 hover:bg-amber-700 font-black uppercase">
...
function StatChip({ label, value, highlight, }) {
    return (<div className={`rounded-md border px-3 py-2 ${highlight ? "border-primary/50 bg-primary/10" : "border-border bg-muted/40"}`}>
      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">{label}</p>
      <p className={`font-bold ${highlight ? "text-primary" : ""}`}>{value}</p>
    </div>);
}
function OpenShiftDialog({ open, onClose, onOpened, }) {
    ...
    const submit = async () => {
        ...
        if (existing) {
            setSaving(false);
            toast.error(`Já existe um turno aberto (${existing.shift_type === "almoco" ? "Almoço" : "Noite"}). Feche-o antes de abrir outro.`);
            onOpened();
            return;
        }
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="grid gap-3">
          ...
          <div className="grid gap-1.5">
            ...
            <Input inputMode="decimal" placeholder="0,00" value={cash} onChange={(e) => setCash(e.target.value)}/>
          </div>
        </div>
        <DialogFooter>
...
function CloseShiftDialog({ open, shift, canClose, pendingActive, awaitingPayment, agentUrl, onClose, onClosed, sendShiftEmail, }) {
    const [saving, setSaving] = useState(false);
    const submit = async () => {
        if (!shift)
            return;
        ...
        try {
            const report = await createShiftReport({ ...shift, closed_at: closedAt });
            try {
                ...
                toast.warning(`Turno fechado. Relatório salvo, mas a impressão falhou: ${e.message}. Reimprima na aba Relatórios.`);
            }
        }
        catch (e) {
        ...
        try {
            const res = await sendShiftEmail({ data: { shiftId: shift.id } });
    ...
    return (<Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        ...
        {!canClose && (<ul className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm space-y-1">
            {pendingActive > 0 && (<li>• {pendingActive} pedido(s) ainda em andamento (não entregues).</li>)}
            {awaitingPayment > 0 && (<li>• {awaitingPayment} pedido(s) entregues aguardando confirmação de pagamento.</li>)}
          </ul>)}
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>
            Cancelar
...
function ConfirmPaymentDialog({ order, shiftId, lastMotoboyId, onClose, onConfirm, isPending, }) {
    const [method, setMethod] = useState("dinheiro");
    const [motoboyId, setMotoboyId] = useState("none");
    useEffect(() => {
        if (order)
            setMethod(order.payment_method ?? "dinheiro");
        if (order)
            setMotoboyId(order.motoboy_id ?? lastMotoboyId ?? "none");
    }, [order, lastMotoboyId]);
    ...
    return (<Dialog open={!!order} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
        ...
        <div className="grid gap-3">
          <div className="grid gap-1.5">
            ...
            <Select value={method} onValueChange={setMethod}>
              ...
              <SelectContent>
                {Object.entries(PAYMENT_LABELS).map(([k, label]) => (<SelectItem key={k} value={k}>
                    {label}
                  </SelectItem>))}
              </SelectContent>
            </Select>
          ...
          <div className="grid gap-1.5">
            ...
            <Select value={motoboyId} onValueChange={setMotoboyId}>
              ...
              <SelectContent>
                ...
                {(shiftMotoboys ?? []).map((m) => (<SelectItem key={m.motoboy_id} value={m.motoboy_id}>
                    {m.motoboys?.name ?? "Motoboy"}
                  </SelectItem>))}
              </SelectContent>
            </Select>
            {(shiftMotoboys ?? []).length === 0 && (<p className="text-xs text-muted-foreground">
                Nenhum motoboy escalado no turno — cadastre na aba Motoboys.
              </p>)}
          </div>
          {order && (<p className="text-sm text-muted-foreground">
              Total do pedido:{" "}
              <strong>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onConfirm(method, motoboyId === "none" ? null : motoboyId)} disabled={isPending}>
            {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
...
function MenuAvailabilityPanel() {
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
        <Input placeholder="Buscar produto…" value={search} onChange={(e) => setSearch(e.target.value)} className="sm:max-w-xs"/>
        <p className="text-xs text-muted-foreground">
      ...
      {data.cats.map((cat) => {
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {catItems.map((item) => (<Card key={item.id} className="flex items-center justify-between gap-3 p-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold ${item.is_available ? "text-green-600" : "text-muted-foreground"}`}>
                      {item.is_available ? "Disponível" : "Indisponível"}
                    </span>
                    <Switch checked={item.is_available} onCheckedChange={(v) => toggle.mutate({ id: item.id, value: v })}/>
                  </div>
                </Card>))}
            </div>
          </section>);
...
function ReportsPanel({ agentUrl }) {
    const qc = useQueryClient();
    ...
    const types = new Set(shiftReports.map((r) => r.shift_type));
    const bothClosed = types.has("almoco") && types.has("noite");
    const printShift = async (r) => {
        try {
    ...
    const generateDaily = useMutation({
        mutationFn: async () => {
            ...
            try {
                const res = await sendDailyReportEmail({ data: { date } });
                if (res?.reason === "no_recipients") {
    ...
    const printDaily = async () => {
        if (!daily)
            return;
        try {
            await sendBytesToPrinter(agentUrl, buildDailyReportBytes(daily));
            await markPrinted("daily_reports", daily.id);
            qc.invalidateQueries({ queryKey: ["daily-report", date] });
    ...
    return (<div className="space-y-6">
      <Card className="p-4 bg-muted/30">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input id="rep-date" type="date" value={date} max={todayISO()} onChange={(e) => setDate(e.target.value)} className="w-40"/>
          </div>
        </div>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          {shiftReports.map((r) => (<Card key={r.id} className="p-4">
              ...
              <div className="mt-3 space-y-1 text-sm">
                ...
                {Object.entries(r.totals_by_payment ?? {}).map(([k, v]) => (<div key={k} className="flex justify-between text-muted-foreground">
          ...
          {shiftReports.length === 0 && (<Card className="p-8 text-center text-muted-foreground">
          ...
          {daily ? (<>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {daily.shifts_count} turnos · {daily.orders_count} pedidos
                  </p>
                </div>
                <p className="text-xl font-black text-primary">
                  {money(Number(daily.total_revenue))}
                </p>
              </div>
              <div className="mt-3 space-y-1 text-sm">
                {Object.entries(daily.totals_by_payment ?? {}).map(([k, v]) => (<div key={k} className="flex justify-between text-muted-foreground">
                    <span>{REPORT_PAYMENT_LABELS[k] ?? k}</span>
                    <span>{money(Number(v))}</span>
                  </div>))}
              </div>
              ...
              <Button className="mt-4" disabled={!bothClosed || generateDaily.isPending} onClick={() => generateDaily.mutate()}>
