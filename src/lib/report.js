...
const pad = (l, r, w = 48) => l + ".".repeat(Math.max(1, w - l.length - r.length)) + r;
export const money = (n) => `R$ ${(n || 0).toFixed(2).replace('.', ',')}`;
...
function paymentBlock(out, totals) {
    ...
    if (entries.length === 0)
        out.push(...line("Nenhum pagamento confirmado"));
    for (const [k, v] of entries) {
        out.push(...line(pad(PAYMENT_LABELS[k] ?? k, money(Number(v)))));
    }
}
function itemsBlock(out, items) {
...
function combosBlock(out, combos) {
...
function motoboysBlock(out, motoboys) {
    const list = motoboys ?? [];
    if (list.length === 0)
        return;
    out.push(...line(""));
...
function deletedOrdersBlock(out, deletedOrders) {
...
export function buildShiftReportBytes(r) {
    const out = [];
...
export function buildDailyReportBytes(r) {
    const out = [];
...
export async function sendBytesToPrinter(agentUrl, bytes) {
...
export function reportToText(r) {
    ...
    lines.push("");
    for (const s of r.shifts_summary) {
        lines.push(`${shiftLabel(s.shift_type)} (${hhmm(s.opened_at)}-${hhmm(s.closed_at)}) — ${s.orders_count} pedidos — ${money(s.total_revenue)}`);
    }
    lines.push("");
    ...
    for (const [k, v] of Object.entries(r.totals_by_payment ?? {})) {
        if (Number(v) > 0)
            lines.push(`- ${PAYMENT_LABELS[k] ?? k}: ${money(Number(v))}`);
    }
    return lines.join("\n");
}
